...
import com.homecoolink.data.Message;
...
//		FList.getInstance().searchLocalDevice();
	...
	public void ACK_vRetGetDefenceStates(String contactId,int msgId, int result) {
		...
		if(result==Constants.P2P_SET.ACK_RESULT.ACK_NET_ERROR){
			FList.getInstance().setDefenceState(contactId, Constants.DefenceState.DEFENCE_STATE_WARNING_NET);
			Intent i = new Intent();   
			...
			FList.getInstance().setDefenceState(contactId, Constants.DefenceState.DEFENCE_STATE_WARNING_PWD);
			Intent i = new Intent();   
			...
			FList.getInstance().setDefenceState(contactId, Constants.DefenceState.DEFENCE_NO_PERMISSION);
		}
	}
	/* 获取设备各种设置回调
	 * 结束
	...
	public void vRetBindAlarmIdResult(int result, int maxCount, String[] data) {
		...
		if(result==1){
			...
			alarmId.putExtra("user",currentUser);
			alarmId.putExtra("pwd", currentPwd);
	...
	public static int defenceAreaNum = -1;
	@Override
	public void vRetClearDefenceAreaState(int result) {
    	...
    	i.putExtra("result", result);
    	if (defenceAreaNum!=-1) {
    		i.putExtra("defenceAreaNum", defenceAreaNum);			
		}
	...
	// 
	@Override
	public void vRetDefenceAreaResult(int result, ArrayList<int[]> data, int group,
			int item) {
		// TODO Auto-generated method stub
	...
	public void vRetGetRemoteDefenceResult(String contactId,int state) {
		...
		if(state==Constants.P2P_SET.REMOTE_DEFENCE_SET.ALARM_SWITCH_ON){
			FList.getInstance().setDefenceState(contactId, Constants.DefenceState.DEFENCE_STATE_ON);
		}else{
			FList.getInstance().setDefenceState(contactId, Constants.DefenceState.DEFENCE_STATE_OFF);
		}
...
//		i.setAction(SysMsgActivity.REFRESH);
	...
	@Override
	public void ACK_vRetGetAlarmRecord(int msgId, int state) {
		// TODO Auto-generated method stub
		Intent i = new Intent();
		i.setAction(Constants.P2P.ACK_RET_GET_ALAEM_RECORD);
		i.putExtra("state", state);
		MyApp.app.sendBroadcast(i);
	}

	@Override
	public void vRetGetAlarmRecord(int number, int index, ArrayList<AlarmRecord> list) {
		// TODO Auto-generated method stub
		
		Intent i = new Intent();
		i.setAction(Constants.P2P.RET_GET_ALARM_RECORD);
