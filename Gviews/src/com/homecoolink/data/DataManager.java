...
public class DataManager {
	...
	public static synchronized List<DefenceAreaName> findDefenceAreaNameAll(
			Context context) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			DefenceAreaNameDB defencearenameDB = new DefenceAreaNameDB(db);
			List<DefenceAreaName> lists = defencearenameDB
					.findDefenceAreaNameAll();
			db.close();
			return lists;
		}
	}
	...
	public static synchronized void insertDefenceAreaName(Context context,
			DefenceAreaName dan) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			DefenceAreaNameDB defenceAreaNameDB = new DefenceAreaNameDB(db);
			defenceAreaNameDB.insert(dan);
			db.close();
		}
	}


	public static synchronized String checkDefenceAreaName(Context context,
			String groupIJ) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			DefenceAreaNameDB defencearenameDB = new DefenceAreaNameDB(db);
			String resut = defencearenameDB.check(groupIJ);
			db.close();
			return resut;
		}
	}
	...
	public static synchronized void upDefenceAreaName(Context context,
			DefenceAreaName dan) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			DefenceAreaNameDB defencearenameDB = new DefenceAreaNameDB(db);
			defencearenameDB.update(dan);
			db.close();
		}
	}
	
	public static synchronized String getMinDefenceAreaItem(Context context,
			String groupI) {
		...
		synchronized (DataManager.class) {
			...
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			DefenceAreaNameDB defencearenameDB = new DefenceAreaNameDB(db);
			String reString= defencearenameDB.GetMinItem(groupI);
			db.close();
			return reString;
		}
	}
	...
	public static synchronized void delDefenceAreaName(Context context,
			String groupIJ) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			...
			DefenceAreaNameDB defencearenameDB = new DefenceAreaNameDB(db);
			defencearenameDB.delete(groupIJ);
			db.close();
		}
	}
	 ...
	 * AlarmRecordDB
	 */
	public static synchronized int updateAlarmRecordReadById(Context context,
			int id) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			...
			int res = alarmRecordDB.updateReadById(id);
			db.close();
			Intent read = new Intent();
			read.setAction("com.homecoolink.AlarmRecord_Read");
			context.sendBroadcast(read);
			return res;
		}
	}
	public static synchronized void updateAlarmRecordRead(Context context,
			String deviceId) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			 alarmRecordDB.updateRead(deviceId);
			db.close();
			Intent read = new Intent();
			read.setAction("com.homecoolink.AlarmRecord_Read");
			context.sendBroadcast(read);
	...
	public static synchronized List<AlarmRecord> findUnReadAlarmRecordByActiveUser(
			Context context, String activeUserId) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			...
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			List<AlarmRecord> lists = alarmRecordDB
					.findUnReadByActiveUserId(activeUserId);
			db.close();
			return lists;
		}
	}

	public static synchronized List<AlarmRecord> findAlarmRecordByDeviceID(
			Context context, String activeUserId, String dvid) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			List<AlarmRecord> lists = alarmRecordDB.findByDeviceId(
					activeUserId, dvid);
			db.close();
			return lists;
		}
	...
	public static synchronized List<AlarmRecord> findUnReadAlarmRecordByDeviceID(
			Context context, String activeUserId, String dvid) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			List<AlarmRecord> lists = alarmRecordDB.findUnReadByDeviceId(
					activeUserId, dvid);
			db.close();
			return lists;
		}
	...
	public static synchronized List<AlarmRecord> findAlarmRecordByGroup(
			Context context, String activeUserId, String dvid) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			List<AlarmRecord> lists = alarmRecordDB.findByGroup(activeUserId,
					dvid);
			db.close();
			return lists;
		}
	...
	public static synchronized Boolean Exists(
			Context context, String activeUserId, String dvid, int atype,
		...
		synchronized (DataManager.class) {
			...
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			Boolean res = alarmRecordDB.Exists(activeUserId, dvid, atype, alartime, g, i);
			db.close();
			return res;
		}
	}
	...
	public static synchronized void insertAlarmRecords(Context context,
			ArrayList<com.homecoolink.data.AlarmRecord >  alarmRecords) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			alarmRecordDB.inserts(alarmRecords);
			db.close();
			Intent read = new Intent();
			read.setAction("com.homecoolink.AlarmRecord_Read");
			context.sendBroadcast(read);
	...
	public static synchronized void deleteAlarmRecordByDeviceId(
			Context context, String activeUserId, String dvid) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			AlarmRecordDB alarmRecordDB = new AlarmRecordDB(db);
			alarmRecordDB.deleteByDeviceId(activeUserId, dvid);
			db.close();
			Intent read = new Intent();
			read.setAction("com.homecoolink.AlarmRecord_Read");
			context.sendBroadcast(read);
	...
	public static synchronized void deleteAlarmRecordByActiveUser(
			Context context, String activeUserId) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
	...
	public static synchronized ArrayList<DeviceAlarmRecord> findDeviceAlarmRecordByDeviceId(
			Context context, String id) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			DeviceAlarmRecordDB AlarmRecordDB = new DeviceAlarmRecordDB(db);
			ArrayList<DeviceAlarmRecord> list = AlarmRecordDB
					.findByDeviceId(id);
			db.close();
			return list;
		}
	...
	public static synchronized void insertDeviceAlarmRecord(Context context,
			DeviceAlarmRecord record) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
	...
	public static synchronized void clearDeviceAlarmRecord(Context context,
			String deviceId) {
		synchronized (DataManager.class) {
			DBHelper dbHelper = new DBHelper(context, DataBaseName, null,
					DataBaseVersion);
			SQLiteDatabase db = dbHelper.getWritableDatabase();
			DeviceAlarmRecordDB AlarmRecordDB = new DeviceAlarmRecordDB(db);
			AlarmRecordDB.deleteByDeviceId(deviceId);
			db.close();
		}
	}
