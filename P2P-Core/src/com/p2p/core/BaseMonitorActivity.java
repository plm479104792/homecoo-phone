...
public abstract class BaseMonitorActivity extends BaseCoreActivity{
	...
	private class GestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
				...
				if (ishorizontal) {
					...
					if (Math.abs(distance) > MyUtils.dip2px(BaseMonitorActivity.this, MINX)) {
						if (distance > 0) {
					...
					if (Math.abs(distance) > MyUtils.dip2px(BaseMonitorActivity.this, MINY)) {
						if (distance > 0) {
	...
	private Handler mHandler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			case MSG_SHOW_CAPTURERESULT:
				if (msg.arg1 == 1) {
					onCaptureScreenResult(true);
				}
				else {
					onCaptureScreenResult(false);
				}
				break;
			default:
				break;
	...
	protected abstract void onCaptureScreenResult(boolean isSuccess);
}
