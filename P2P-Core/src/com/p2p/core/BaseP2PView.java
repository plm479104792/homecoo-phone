...
public abstract class BaseP2PView extends SurfaceView {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			mode = MODE.NONE;
			/** 执行缩放还原 **/
			//if (this.getWidth()<this.getCurrentWidth()&&this.getHeight()<this.getCurrentHeight()) {
	...
	int getCenterX(MotionEvent event) {
		//Log.e("Gview","x0" + (int)event.getX(0) + " x1" + (int)event.getX(1) +" y0 "+ (int)event.getY(0) + " y1 "+ (int)event.getY(1) );
