...
public class AdapterSlidingList extends BaseAdapter {
	private LayoutInflater mInflater;

	private List<AdapterSlidingItem> mItems;

	public AdapterSlidingList(Context context, List<AdapterSlidingItem> list) {
	...
	public View getView(int position, View convertView, ViewGroup par) {
		...
		if (mItems.get(position).isGroup()) {
			holder.file_res.setImageResource(ETGlobal.mGroupImages[mItems.get(
					position).getRes()]);
		}
		else
		{
			holder.file_res.setImageResource(ETGlobal.mDeviceImages[mItems.get(
					position).getRes()]);
		}
